...
结果：
命令中只读、只写或者只有查询的时候，只会调用一个工具，如果要先写再读的话，就会调用多个工具。这四个例子的多个工具调用的顺序和结果正确。
...
在这个上下文中：当工具执行结果太大（超过 maxResultSizeChars 阈值），系统不会把完整内容直接传给模型，而是把它写入一个文件（落盘），模型只能看到一个简短预览 + 文件路径。模型如果需要完整内容，可以再去读那个文件。

这是一种内存保护机制——防止巨大的工具输出撑爆上下文窗口
